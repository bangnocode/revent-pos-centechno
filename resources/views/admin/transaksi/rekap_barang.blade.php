...
<div class="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
    <div>
        <h2 class="text-2xl font-bold text-gray-800">Rekap Penjualan Per Barang</h2>
        <p class="text-sm text-gray-500">Rekapitulasi total penjualan dan laba rugi per barang per tanggal</p>
    </div>
</div>

<!-- Filters -->
<div class="bg-white rounded-lg shadow-sm border border-gray-100 p-4 mb-6">
    <form action="{{ route('admin.transaksi.rekap-barang') }}" method="GET" class="flex flex-col md:flex-row gap-4">
        ...
        <div class="flex-1">
             <label class="block text-xs font-medium text-gray-600 mb-1">Cari Barang</label>
             <div class="relative">
                <input type="text" name="search" value="{{ $search }}" placeholder="Cari nama barang atau kode barang..." 
...
<div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
    ...
    <div class="bg-white p-4 rounded-lg shadow-sm border border-gray-100">
        <p class="text-xs text-gray-500 font-semibold uppercase">Total Terjual (Semua Barang)</p>
...
<div class="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden">
        ...
        <table class="w-full text-sm text-left">
            <thead class="bg-gray-50 text-gray-600 font-medium border-b border-gray-200">
                <tr>
                    ...
                    <th class="px-4 py-3 text-center">Total Terjual (Qty)</th>
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50 transition-colors">
                    ...
                    <td class="px-4 py-4 text-center font-bold">
                        {{ number_format($item->total_qty, 0, ',', '.') }} <span class="text-[10px] text-gray-400 capitalize">{{ $item->satuan }}</span>
                    </td>
    ...
    <div class="px-6 py-4 border-t border-gray-100 bg-gray-50">
        {{ $rekap->appends(['start_date' => $startDate, 'end_date' => $endDate, 'search' => $search])->links() }}
    </div>
    @endif
</div>
@endsection
